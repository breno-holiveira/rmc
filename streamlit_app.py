...
# HTML embutido com estilos refinados
template = f"""
<!DOCTYPE html>
<html lang=\"pt-BR\">
<head>
...
<style>
  html, body {{ height: 100%; margin: 0; font-family: 'Segoe UI', sans-serif; background: linear-gradient(120deg, #f5f8fc, #e6ecf5); overflow: hidden; }}
  #sidebar {{ position: fixed; top: 0; left: 0; width: 240px; height: 100vh; background: rgba(255,255,255,0.80); backdrop-filter: blur(12px); border-right: 1px solid rgba(71,88,110,0.2); padding: 16px; overflow-y: auto; scroll-behavior: smooth; z-index: 10; box-shadow: 2px 0 8px rgba(0,0,0,0.06); }}
  #sidebar::-webkit-scrollbar {{ width: 8px; }}
  #sidebar::-webkit-scrollbar-thumb {{ background-color: rgba(120,140,160,0.3); border-radius: 8px; }}
  #sidebar:hover::-webkit-scrollbar-thumb {{ background-color: rgba(120,140,160,0.5); }}
  #sidebar h2 {{ margin: 0 0 12px; color: #35455e; font-size: 17px; }}
  #search {{ width: 100%; padding: 8px; border: none; border-radius: 10px; background: rgba(255,255,255,0.6); backdrop-filter: blur(6px); margin-bottom: 12px; font-size: 14px; color: #35455e; }}
  #list div {{ padding: 8px; border-radius: 8px; cursor: pointer; color: #35455e; transition: background 0.2s; }}
  #list div:hover {{ background: rgba(120,140,160,0.15); }}
  #list div.active {{ background: rgba(54,75,102,0.5); color: #fff; font-weight: 600; }}
  #map {{ margin-left: 240px; height: 100vh; position: relative; overflow: hidden; }}
  svg {{ width: 100%; height: 100%; display: block; }}
  .area {{ fill: rgba(180,200,220,0.6); stroke: rgba(71,88,110,0.7); stroke-width: 1; cursor: pointer; transition: all 0.2s ease; }}
  .area:hover {{ fill: rgba(140,160,185,0.7); stroke-width: 1.2; }}
  .area.selected {{ fill: rgba(71,88,110,0.8); stroke: rgba(54,75,102,1); stroke-width: 1.5; }}
  #tooltip {{ position: fixed; background: rgba(44,62,112,0.9); color: white; padding: 6px 10px; font-size: 13px; border-radius: 5px; display: none; pointer-events: none; z-index: 1000; }}
  #info {{ position: absolute; top: 24px; right: 24px; background: rgba(255,255,255,0.85); backdrop-filter: blur(12px); padding: 16px; border-radius: 10px; box-shadow: 0 2px 6px rgba(0,0,0,0.1); max-width: 280px; font-size: 13px; display: none; }}
  #info.visible {{ display: block; }}
  #info h3 {{ margin: 0 0 10px; color: #35455e; }}
...
<div id="sidebar">
  <h2>Municípios</h2>
  <input id="search" placeholder="🔍 Buscar município…" />
  <div id="list"></div>
</div>
<div id="map">
...
st.components.v1.html(template, height=750, scrolling=False)
