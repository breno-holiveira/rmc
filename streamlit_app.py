...
def load_geojson():
    # Carrega shapefile e transforma para EPSG:4326 se necessário
...
# Carregando o HTML do mapa (assumindo que você tem um arquivo 'grafico.html' na pasta)
# Caso queira, pode embutir direto, mas prefiro arquivo separado para manutenção.

html_path = Path("grafico.html")
if html_path.exists():
    html_code = html_path.read_text(encoding="utf-8")
    html_code = html_code.replace("{{geojson}}", geojson_str)
else:
    st.error("Arquivo grafico.html não encontrado na raiz do projeto.")
    st.stop()

# Exibe o mapa interativo com o HTML customizado
