...
<style>
  ...
  #list div.active {{ background-color: #4d648d; color: #fff; font-weight: 600; }}
  #map {{ flex-grow: 1; position: relative; overflow: hidden; }}
  ...
  #info h3 {{ font-size: 20px; font-weight: 700; color: #2c3e70; }}
  #info .grid {{
    display: grid;
  ...
  #info .fonte {{ grid-column: 1 / -1; font-size: 11px; color: #7f8caa; text-align: right; }}
</style>
</head>
<body>
  <div id=\"sidebar\">
...
geo.features.forEach(f => {
  ...
  path.addEventListener("click", e => { e.preventDefault(); select(name); });
...
search.addEventListener("input", e => {
  ...
  if(visibleItems.length === 1) select(visibleItems[0].dataset.name);
});
